...
//	public $module = 'voyagebuilder';
	 ...
	 */
	public $table_element = 'voyagebuilder_voyage';
	 ...
	 */
	public $picto = 'voyage@voyagebuilder';
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->voyagebuilder->voyage->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->voyagebuilder->voyage->voyage_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->voyagebuilder->dir_output.'/voyage/'.$oldref;
				$dirdest = $conf->voyagebuilder->dir_output.'/voyage/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->voyagebuilder->dir_output.'/voyage/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->voyagebuilder->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->voyagebuilder->voyagebuilder_advance->validate))))
		 {
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/voyagebuilder/voyage_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_voyage = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->VOYAGEBUILDER_VOYAGE_ADDON)) {
			$conf->global->VOYAGEBUILDER_VOYAGE_ADDON = 'mod_voyage_standard';
		}

		if (!empty($conf->global->VOYAGEBUILDER_VOYAGE_ADDON)) {
			$mybool = false;

			$file = $conf->global->VOYAGEBUILDER_VOYAGE_ADDON.".php";
			$classname = $conf->global->VOYAGEBUILDER_VOYAGE_ADDON;

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/voyagebuilder/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/voyagebuilder/doc/";
