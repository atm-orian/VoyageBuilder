...
$langs->loadLangs(array("voyagebuilder@voyagebuilder", "other"));
...
$diroutputmassaction = $conf->voyagebuilder->dir_output.'/temp/massgeneration/'.$user->id;
...
$upload_dir = $conf->voyagebuilder->multidir_output[isset($object->entity) ? $object->entity : 1].'/voyage';
...
if (empty($conf->voyagebuilder->enabled)) accessforbidden();
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/voyagebuilder/voyage_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/voyagebuilder/voyage_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	$triggersendname = 'VOYAGEBUILDER_VOYAGE_SENTBYMAIL';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Voyage"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteVoyage'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/voyagebuilder/voyage_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->voyagebuilder->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('voyagebuilder:Voyage', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/voyagebuilder/voyage_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->voyagebuilder->dir_output;
	$trackid = 'voyage'.$object->id;
